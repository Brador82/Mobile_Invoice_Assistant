...
public class DriveHelper {
    ...
    static /* synthetic */ void lambda$uploadFile$1(Context context, Callback callback, File localFile, String mimeType) {
        try {
            ...
            com.google.api.services.drive.model.File uploaded = driveService.files().create(metadata, content).setFields("id, name, webViewLink").execute();
    ...
    private static String getOrCreateFolder(Drive driveService, String folderName) {
        try {
            FileList result = driveService.files().list().setQ("mimeType='application/vnd.google-apps.folder' and name='" + folderName + "' and trashed=false").setFields("files(id, name)").execute();
            ...
            com.google.api.services.drive.model.File created = driveService.files().create(folderMeta).setFields("id").execute();
